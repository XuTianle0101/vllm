...
OUT="${OUT:?Set OUT to an existing result directory}"
...
for mode in $MODES; do
  ...
  uv run --no-project .venv/bin/vllm serve "$MODEL" --served-model-name ksa --host 127.0.0.1 --port 18015 --dtype bfloat16 --enforce-eager --no-enable-prefix-caching --max-model-len "$MAX_MODEL_LEN" --max-num-seqs 8 --max-num-batched-tokens "$ROW_BUDGET" --gpu-memory-utilization "$GPU_MEMORY_UTILIZATION" --additional-config "{\"ksa_cudagraph\":$enabled}" > "$OUT/http-$mode-server.log" 2>&1 &
  server_pid=$!
  ...
  if [ "$ready" = true ]; then
    uv run --no-project .venv/bin/python benchmarks/ksa/v1_serving.py --url http://127.0.0.1:18015 --model ksa --output "$OUT/http-$mode" > "$OUT/http-$mode.log" 2>&1
    rc=$?
  else
    rc=125
  fi
